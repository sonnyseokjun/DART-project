...
#: v4: evidence 상한을 8 → 12로 늘리고 **같은 구간의 중복 인용을 금지**(§5).
#:     v3 요약 140건 중 101건(72%)이 8칸을 꽉 채웠고, 65건은 같은 quote 를 두 번 이상
#:     담고 있었다. 천장에 눌린 채 칸을 중복에 낭비하던 셈이라 둘을 함께 고친다.
PROMPT_VERSION = 'v4'
...
SUMMARY_JSON_SCHEMA = {
    ...
    'properties': {
        ...
        'evidence': {
            'type': 'array',
            'minItems': 1,
            # 상한 8은 병목이었다. v3 요약 140건 중 101건이 정확히 8칸에서 멈췄는데,
            # 수치가 많은 공시(유상증자·대규모 공급계약·정기보고서)는 근거가 8개를
            # 넘긴다. 넘치면 LLM이 근거를 버리고, 그 결과 요약은 맞는데 "인용에 없는
            # 수치" 경고가 붙는다. 요약이 틀려서가 아니라 적을 자리가 없어서 생기는
            # 오탐이므로 상한을 올려 막는다.
            'maxItems': 12,
            'description': (
...
- **같은 구간을 두 항목에 중복해서 담지 마라.** 바로 위 규칙은 *서로 다른* 구간을
  나누라는 뜻이지, 같은 문장을 반복하라는 뜻이 아니다. 한 구간이 수치 여러 개를
  뒷받침한다면 항목 하나에 claim 을 묶어 쓰고, 근거 항목은 하나만 만든다.
  중복은 근거를 늘리지 않으면서 자리만 차지한다.
