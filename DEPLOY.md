...
**2026-08-28 — cron이 처음으로 사람 손 없이 완주했다.**

| 항목 | 값 |
|---|---|
| 파이프라인 | 수집 6건 → 원문 3건 → 요약 3건, 전 구간 무인 |
| 요약 비용 | **$0.0274** (건당 $0.0091) |
| 호스트 메모리 | 491~501MB 사용 |

건당 비용이 예산($0.023)의 40%로 나온 건 **프롬프트 캐시가 56% 적중**했기 때문이다.
같은 공시 유형이 하루에 몰려 들어오면 앞 건이 뒤 건의 캐시가 된다.

메모리는 오히려 전날보다 안정됐지만, **요약 피크는 여전히 추정치다.** 샘플러가
30분 간격이고 요약 배치는 1분 안에 끝나서 겹치질 않는다 — 측정하려면 배치 창에서만
조밀하게 찍어야 한다. 감시 주기를 정할 때 흔히 놓치는 함정이다.
